...
describe("extractText", () => {
  ...
  it("joins page text and reports char count", async () => {
    vi.mocked(unpdf.extractText).mockResolvedValueOnce({ totalPages: 1, text: ["Hello SDS world ".repeat(20)] });
  ...
  it("marks a low-text PDF as likely scanned", async () => {
    vi.mocked(unpdf.extractText).mockResolvedValueOnce({ totalPages: 1, text: ["short"] });
